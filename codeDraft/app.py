...
@app.route("/delete/<givenID>/", methods=["GET", "POST"])
def delete(givenID):
    deleteData(givenID)
...
def data():
    try:
        ...
        query = (
            f"SELECT id, question, answer FROM QuesAns LIMIT {length} OFFSET {start}"
        )
        ...
        for row in data:
            response_data.append(
                list(row)
                + [
                    f'<a href="#/update/{row[0]}" class="editButton" data-toggle="modal" data-target="#modaledit{row[0]}"> Edit </a> <a href="/delete/{row[0]}" class="deleteButton" onclick="return confirm(\'Are you sure you want to delete?\')"> Delete </a>'
                ]
            )
